...
class UpdaterMod(loader.Module):
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                "GIT_ORIGIN_URL",
                "https://github.com/qqsharki4/Heroku",  # ← Изменено на ваш репозиторий
    ...
    def get_latest(self) -> str:
        try:
            return next(
                git.Repo().iter_commits(f"origin/main", max_count=1)  # ← Изменено на main
            ).hexsha
        except Exception:
            return ""
    ...
    async def poller(self):
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            m = await self.inline.bot.send_photo(
                self.tg_id,
                "https://raw.githubusercontent.com/qqsharki4/Heroku/refs/heads/main/assets/heroku-update.png",  # ← Изменено на ваш репозиторий
                caption=self.strings("update_required").format(
                    utils.get_git_hash()[:6],
                    '<a href="https://github.com/qqsharki4/Heroku/compare/{}...{}">{}</a>'.format(  # ← Изменено на ваш репозиторий
    ...
    async def download_common(self):
        try:
            ...
            origin.fetch()
            repo.create_head("main", origin.refs.main)  # ← Изменено на main
            repo.heads.main.set_tracking_branch(origin.refs.main)  # ← Изменено на main
            repo.heads.main.checkout(True)  # ← Изменено на main
            return False
                ...
                UpdateDialogFilterRequest(
                    ...
                    DialogFilter(
                        ...
                        include_peers=[
                            ...
                            # Убраны официальные чаты оригинального репозитория
                        ],
                        emoticon="🐱",
                        exclude_peers=[],
                        contacts=False,
